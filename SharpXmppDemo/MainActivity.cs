...
{
    [Activity(Label = "SecureComm", MainLauncher = true, Icon = "@drawable/icon")]
    public class MainActivity : Activity
    {
        ...
        private string password = "Th1nkp4d!";
        private string jid = "ghost@securecomm.xyz";
        private string messageRecipient = "ghost@securecomm.xyz"; // Send a message to myself
    ...
    public class ConnectActivityReceiver : BroadcastReceiver
    {
        public override void OnReceive(Context context, Intent intent)
        {
            if (intent.GetBooleanExtra(ServiceMessage.OK, true))
            {
                Log.Info("SecureComm", this.Class.ToString() + "Service call has succeeded");
            }
            else
            {
                Log.Error("SecureComm", this.Class.ToString() + "Service call failed");
            }
        }
    }
    ...
    public class MesssageActivityReceiver : BroadcastReceiver
    {
        public override void OnReceive(Context context, Intent intent)
        {
            if (intent.GetBooleanExtra(ServiceMessage.OK, true))
            {
                Log.Info("SecureComm", this.Class.ToString() + "Message was sent");
            }
            else
            {
                Log.Error("SecureComm", this.Class.ToString() + "Message failed");
            }
        }
    }
}
