...
namespace SecureComm
{
    ...
    public class WakefulXmppReceiver : Android.Support.V4.Content.WakefulBroadcastReceiver
    {
        ...
        {
            Android.Util.Log.Info("SecureComm", this.Class.ToString() + "Starting wakefull receiver Alarm");
            ...
            {
                Android.Util.Log.Info("SecureComm", this.Class.ToString() + "Not waked up since we are offline anyway!");
            }
        }
    }
    ...
    public class InitBootReceiver : BroadcastReceiver
    {
        ...
        public override void OnReceive(Context context, Intent intent)
        {
            Log.Info("SecureComm", this.Class.ToString() + " InitBootReceiver Activated on Boot");
    ...
    public class ConnectivityChangeReceiver : Android.Support.V4.Content.WakefulBroadcastReceiver
    {
        ...
        public override void OnReceive(Context context, Intent intent)
        {
            ...
            if (noConnection)
                Log.Debug("SecureComm", this.Class.ToString() + " No Connection detected event");
            else
            {
                Log.Debug("SecureComm", this.Class.ToString() + "Has Connection detected event");
    ...
    public class WakeHolder : IWakeAble, IDisposable
    {
        ...
        public void Remove()
        {
            ...
            {
                Log.Error("SecureComm", ".MultiDebug: " + e.StackTrace + e.ToString());
            }
        }
    ...
    internal class Utils
    {
        ...
        public static void SetConnectionAlarm(Context act)
        {
            if (!Utils.IsAlarmSet(act))
            {
                ...
                Android.Util.Log.Info("SecureComm", act.Class.ToString() + " Setting Wakefull Alarm");
            }
            else
            {
                Log.Info("SecureComm", act.Class.ToString() + " Wakefull Alarm already set");
            }
        }
        ...
        {
            Log.Info("SecureComm", "IsOnline - Checking if Online");
        ...
        public static bool IsServiceActive(Activity act = null)
        {
            ...
            try
            {
                ...
                {
                    Log.Error("SecureComm", "IsServiceActive Stateholder is Instance Null");
            ...
            {
                Log.Error("SecureComm", "IsServiceActive: Null argument exception" + e.StackTrace + e.ToString());

                return false;
            }
            catch (ArgumentException e)
            {
                Log.Error("SecureComm", "IsServiceActive: Unknown argument exception" + e.StackTrace + e.ToString());

                return false;
            }

            Log.Debug("SecureComm", "IsServiceActive Stateholder is Instance Exists, everything OK");
