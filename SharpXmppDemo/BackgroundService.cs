...
namespace SecureComm
{
    ...
    public class BackgroundService : StickyIntentService
    {
        ...
        {
            base.OnDestroy();
            Log.Info("SecureComm", this.Class.ToString() + "I'm being destroyed :(");
        }
        ...
        public override void OnCreate()
        {
            ...
            Log.Info("SecureComm", this.Class.ToString() + " I'm being created :)");
        }
        ...
        protected override void OnHandleIntent(Intent intent)
        {
            ...
            if (intent == null)
            {
                Log.Info("SecureComm", this.Class.ToString() + " StartSticky reinitiated Intent ");
            ...
            Log.Info("SecureComm", this.Class.ToString() + " Processing action " + intent.Action.ToString());
        ...
        {
            Log.Info("SecureComm", this.Class.ToString() + " Accepted intend to send message");
            ...
            Log.Info("SecureComm", this.Class.ToString() + " Moment: " + message + " is send");
            ...
            if (XmppConnection != null)
            {
                ...
                {
                    Log.Error("SecureComm", this.Class.ToString() + e.StackTrace + "\n" + e.ToString());
            ...
            if ((XmppConnection != null))
            {
                if (!XmppConnection.isConnected())
                {
                    try
                    {
                        ...
                        {
                            Log.Info("SecureComm", this.Class.ToString() + "Attempting to Connect from Background Service");

                            XmppConnection.Connect();
                        }
                        else
                        {
                            Log.Info("SecureComm", this.Class.ToString() + "No network, will not proceed to Connect from Background Service");
                        }
                    }
                    catch (Exception e)
                    {
                        Log.Error("SecureComm", this.Class.ToString() + "Error at Connection, Exception Raised during reconnection" + e.ToString() + e.StackTrace);
                    }
                ...
                else
                {
                    try
                    {
                        ...
                        if (Utils.IsOnline(Android.App.Application.Context))
                        {
                            Log.Info("SecureComm", this.Class.ToString() + "Attempting to Ping from Background Service");

                            XmppConnection.PingServer();
                        }
                        else
                        {
                            Log.Info("SecureComm", this.Class.ToString() + "No network, will not proceed to Ping from Background Service");
                        }
                    }
                    catch (Exception e)
                    {
                        Log.Error("SecureComm", this.Class.ToString() + "Error at Pinging, Exception Raised during reconnection" + e.ToString() + e.StackTrace);
                    }
        ...
        private Intent init(Intent intent)
        {
            ...
            Log.Info("SecureComm", this.Class.ToString() + "Accepted intend to init from form-provided connection strings");
            ...
            {
                Log.Error("SecureComm", this.Class.ToString() + "Error at updating TrustVillage" + e.ToString() + e.StackTrace);
    ...
    public class AndroidDispatcher : IUIThreadDispatcher
    {
        ...
        public void Invoke(Action action)
        {
            ...
            Log.Info("SecureComm", ".MultiDebug: " + "Invoke was called for action: " + action.ToString());
        ...
        {
            Log.Info("SecureComm", ".MultiDebug: " + message);
        }
    }
}